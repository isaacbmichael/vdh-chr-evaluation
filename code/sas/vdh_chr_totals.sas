 ...
 *   2) Open this file in SAS 9.4 (with SAS/GRAPH licensed/available). Tested on SAS 9.4M7.
 *   3) Edit the two paths below: IN_CSV_PATH (input CSV) and OUT_PDF_PATH (where to save PDF).
 ...
 *   • If you replace the synthetic CSV with real data, ensure all approvals and privacy
 ...
 *   • Code: MIT License © 2025 Isaac B. Michael. See LICENSE (MIT) in repo.
 ...
 *
 * CONTRIBUTING / ISSUES
 *   • Open an Issue or Pull Request on GitHub (set REPO_URL below if you want it on the PDF).
 *
...
/* Unified text sizing (cell units) — same visual size, just crisper */
...
/* ------- Percent-high / note box placement (slightly larger boxes) ------- */
%let ANNO_BAR_X1      = 4;    /* left edge  (was 5)  */
%let ANNO_BAR_X2      = 20;   /* right edge (was 19) */
%let ANNO_BAR_YTOP    = 83;   /* top edge   (was 82) */
%let ANNO_BAR_YBOT    = 69;   /* bottom     (was 70) */
%let ANNO_BAR_TEXT_X  = 12;   /* text X (centered)   */
%let ANNO_BAR_LINE2_Y = 75;   /* big % line (was 75) */

%let NOTE_X1          = 4;    /* was 5  */
%let NOTE_X2          = 20;   /* was 19 */
%let NOTE_YTOP        = 83;   /* was 82 */
%let NOTE_YBOT        = 69;   /* was 70 */
%let NOTE_TEXT_X      = 12;
...
/* Force vector output; hard-reset graphics and fonts every run */
ods _all_ close;
...
/* Helper for consistent chart title style (height˜1.75 per prior style) */
%macro bigtitle(text);
...
/*===============================================================================
= DATA IMPORT (CSV)
  - Replace IN_CSV_PATH if you move the CSV.
  - This program expects headers that match variable names referenced below.
===============================================================================*/
...
= VARIABLE LABELS + TYPE NORMALIZATION (make ALL Q* character)
  - This lets us safely map response labels using arrays (no mixed types).
===============================================================================*/
...
/*===============================================================================
= COVER PAGE (executive-friendly) — airy title/subtitle, still fits on one page
  - Increased title/subtitle font sizes + extra spacer rows.
  - Adds plain-English provenance: “Based on actual report… synthetic dataset.”
...
data title_page;
  ...
  A="Summary:";                                       output;
  A="This example mirrors the CHR program’s report structure, variable harmonization,"; output;
  A="and analysis logic for totals and subgroup charts. All results are generated from"; output;
  A="a fully SYNTHETIC dataset that imitates realistic distributions—no real responses."; output;
  ...
  A="Licensing:";                                     output;
  A="• Code: MIT License © 2025 Isaac B. Michael";    output;
  A="• Reports & Documentation: Creative Commons Attribution 4.0 (CC BY 4.0)"; output;

  A=" ";                                              output;

  A="Attribution & Contact:";                         output;
  A="Please credit Isaac B. Michael when reusing and include links to both licenses."; output;

  A=" ";                                              output;

  /* Removed the 'Version Info:' lines per request */
...
/* Mask the comma in the longlabel */
